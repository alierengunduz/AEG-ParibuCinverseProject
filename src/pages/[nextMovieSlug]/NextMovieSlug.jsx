    ...
    const onFinish = (values) => {
      ...
      useEffect(() => {
        const fetchMovieData = async () => {
          try {
            const res = await fetch("/YakindaData.json"); // JSON dosyanızın yolu
            const data = await res.json();
            const movie = data.find((movie) => movie.title === NextMovieSlug);
    
            if (movie) {
              setMovieData(movie);
            } else {
              console.error(`Movie with slug "${NextMovieSlug}" not found.`);
            }
          } catch (error) {
            console.error(error);
          }
              ...
              <div className="flex flex-col gap-y-2 mt-5">
                <div className="flex items-center gap-x-3">
                  <img
                    ...
                    alt=""
                  />
                  <span>Vizyon Tarihi: {movieData.date}</span>
                </div>
                <div className="flex items-center gap-x-3">
                  <img
                    ...
                    alt=""
                  />
                  <span>Tür: {movieData.action}</span>
                </div>
                <div className="flex items-center gap-x-3">
                  <img
