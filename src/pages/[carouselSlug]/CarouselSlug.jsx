    ...
    const onFinish = (values) => {
      ...
      useEffect(() => {
        const fetchMovieData = async () => {
          try {
            const res = await fetch("/CarouselData.json"); // JSON dosyanızın yolu
            const data = await res.json();
            const movie = data.find((movie) => movie.title === CarouselSlug);
    
            if (movie) {
              setMovieData(movie);
            } else {
              console.error(`Movie with slug "${CarouselSlug}" not found.`);
            }
          } catch (error) {
            console.error(error);
          }
